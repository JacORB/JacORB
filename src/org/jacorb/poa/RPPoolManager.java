...
public class RPPoolManager {
	...
	synchronized protected RequestProcessor getProcessor() {
		...
		if (pmListener != null) pmListener.processorRemovedFromPool(rp, pool.size(), pool_size);
		return rp;
	}
	private void init() {
	...
	synchronized protected void releaseProcessor(RequestProcessor rp) {
		...
		// notify a pool manager listener		
		if (pmListener != null) pmListener.processorAddedToPool(rp, pool.size(), pool_size);		
	}
