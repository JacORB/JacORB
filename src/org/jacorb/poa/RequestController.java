...
public class RequestController extends Thread {
	...
	/* a singleton for all POA's in one virtual machine if SINGLE_THREAD_MODEL is in use */
	private static RPPoolManager 	singletonPoolManager;

	private LogTrace logTrace;	
	// stores all active requests	
	private Hashtable 				activeRequestTable; 				// RequestProcessor -> oid
	// for synchronisation with the object deactiviation process
	private Vector 					deactivationList = new Vector(); 	// oid's
	...
	RequestController(POA _poa, org.jacorb.orb.ORB _orb, AOM _aom, LogTrace _logTrace) {
		poa = _poa;
		aom = _aom;
		orb = _orb;
		logTrace = _logTrace;
		...
		requestQueue = new RequestQueue(this, logTrace);
	 	activeRequestTable = poa.isSingleThreadModel() ? new Hashtable(1) : new Hashtable(Environment.threadPoolMax());
		getPoolManager();
	...
	synchronized void freeObject(byte[] oid) {
		deactivationList.removeElement(POAUtil.objectId_to_string(oid));	
	}
	AOM getAOM() {
		return aom;
	}
	...
	RPPoolManager getPoolManager() {		
		if (poolManager == null) {
			if (poa.isSingleThreadModel()) {
				...
				poolManager = new RPPoolManager(orb.getPOACurrent(), 
	...
	private void processRequest(ServerRequest request) throws ShutdownInProgressException, CompletionRequestedException {
		...
		synchronized (this) {
			...
			if (waitForCompletionCalled) { /* state has changed to holding, discarding or inactive */
				logTrace.printLog(2, request, "cannot process request, because waitForCompletion was called");
				throw new CompletionRequestedException();
			}
			
			if (waitForShutdownCalled) { /* poa goes shutdown */
				logTrace.printLog(2, request, "cannot process request, because the poa goes shutdown");
				throw new ShutdownInProgressException();
			}
			
			/* below this point it's save that the poa is active */ 

			if (deactivationList.contains(POAUtil.objectId_to_string(request.objectId()))) {
				if (!poa.isUseServantManager() && !poa.isUseDefaultServant()) {
					logTrace.printLog(0, request, "cannot process request, because object is already in the deactivation process");
			...
			if (servant == null) {
				...
				if (poa.isUseDefaultServant()) {
					if ((servant = poa.defaultServant) == null) {
						logTrace.printLog(0, request, "cannot process request, because default servant is not set");
						throw new org.omg.CORBA.OBJ_ADAPTER();
					}

				} else if (poa.isUseServantManager()) {
					if ((servantManager = poa.servantManager) == null) {
						logTrace.printLog(0, request, "cannot process request, because servant manager is not set");
						throw new org.omg.CORBA.OBJ_ADAPTER();
					}

				// USE_OBJECT_MAP_ONLY is in effect but object not exists
				} else {
					logTrace.printLog(2, request, "cannot process request, because object doesn't exist");
					throw new org.omg.CORBA.OBJECT_NOT_EXIST();
				}
			}

			/* below this point it's save that the request is valid (all preconditions can be met) */
			activeRequestTable.put(request, POAUtil.objectId_to_string(request.objectId()));
		}
		// get and initialize a processor for request processing
		logTrace.printLog(3, request, "trying to get a RequestProcessor");
	...
	void rejectRequest(ServerRequest request, org.omg.CORBA.SystemException exception) {
		if (exception != null) request.setSystemException(exception);
		orb.getBasicAdapter().return_result(request);
		logTrace.printLog(2, request, " request rejected with exception: "+exception);
	}
	/**
	...
	synchronized void resetPreviousCompletionCall() {
		logTrace.printLog(6, "reset a previous completion call");
	...
	public void run() {
		State state;
		ServerRequest request;
		org.omg.CORBA.OBJ_ADAPTER closed_connection_exception = new org.omg.CORBA.OBJ_ADAPTER("connection closed: adapter inactive");
		...
		while (!terminate) {
			...
			if (POAUtil.isActive(state)) {
				...
				/* 	if waitForShutdown was called the RequestController loop blocks for ALL TIME
				 	in waitForQueue (the poa behaves as if he is in holding state now) 
					ATTENTION, it's a lazy synchronisation, a request could be rejected if 
					waitForShutdown was called but couldn't be processed (it's save) */
			waitForQueue();
		}
	}
	/**
	...
	synchronized void waitForCompletion() {
		...
		while (waitForCompletionCalled && !activeRequestTable.isEmpty()) {
			try {
				logTrace.printLog(6, "somebody waits for completion and there are active processors");
				wait();
	 ...
	 * no new requests on this object will started from now on,
	 * a steady stream of incomming requests can keep the object from being deactivated,
	 * a servant may invoke recursive method calls on the object it incarnates and
	 * deactivation should not necessarily prevent those invocations.
	 */
	synchronized void waitForObjectCompletion(byte[] oid) {
		String oidStr = POAUtil.objectId_to_string(oid);			
		while (activeRequestTable.contains(oidStr)) {
			try {
				wait();
			} catch (InterruptedException e) {
			}
		}		
		logTrace.printLog(6, oid, "all active processors for this object have finished");		
		deactivationList.addElement(oidStr);
	}
	/**
	...
	private void waitForQueue() {
		synchronized (queueLog) {
			if ((requestQueue.isEmpty() || poa.isHolding() || waitForShutdownCalled) && !terminate) {
				try {
					logTrace.printLog(6, "the RequestController goes to sleep");
					queueLog.wait();
	...
	synchronized void waitForShutdown() {
		...
		while (waitForShutdownCalled && !activeRequestTable.isEmpty()) {
			try {
				logTrace.printLog(6, "somebody waits for shutdown and there are active processors");
				wait();
			} catch (InterruptedException e) {
			}
		}
