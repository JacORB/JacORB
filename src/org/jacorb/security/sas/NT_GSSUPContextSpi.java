...
public final class NT_GSSUPContextSpi implements GSSContextSpi
{
    ...
    public NT_GSSUPContextSpi (Provider provider, Oid mechOid, GSSNameSpi nameSpi, GSSCredentialSpi credSpi, int lifetime)
    {
    ...
    public byte[] acceptSecContext(InputStream inStream, int inLen) throws GSSException
    {
        ...
        try
        {
            ...
            // check login
            org.omg.GSSUP.InitialContextToken token = GSSUPNameSpi.decode(inBytes);
            if (!checkDomainLogin((new String(token.username)), (new String(token.password)), (new String(token.target_name))))
            {
                Debug.output( 1, "NT Login Failed");
                throw new GSSException(GSSException.DEFECTIVE_CREDENTIAL, 0, "NT Login Failed");
            }
        ...
        catch (IOException e)
        {
            ...
            throw new GSSException(GSSException.DEFECTIVE_CREDENTIAL, 0, "NT Login Failed: " + e);
    ...
    private boolean checkDomainLogin(String loginId, String password, String domainName)
    {
        ...
        try  {
            ...
            //Debug.output(1, "User validated using domain login");
        } catch(Exception ex) {
            Debug.output(1, "Exception serLoginBean:checkDomainLogin): " + ex.getMessage());
            loginStatus = false;
        }
