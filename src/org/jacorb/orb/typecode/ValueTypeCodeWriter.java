 ...
 */
public class ValueTypeCodeWriter extends ComplexTypeCodeWriter
{
                                                                       ...
                                                                       Bounds
    {
        ...
        out.write_short( typeCode.type_modifier() );
        final org.omg.CORBA.TypeCode baseType = typeCode.concrete_base_type();
        if (baseType == null)
        {
            out.write_long (TCKind._tk_null);
        }
        else
        {
            out.write_TypeCode(baseType, recursiveTCMap, repeatedTCMap);
        }
        final int _memberCount = typeCode.member_count();
        out.write_long(_memberCount);
        for( int i = 0; i < _memberCount; i++)
        {
            out.write_string( typeCode.member_name(i) );
            out.write_TypeCode( typeCode.member_type(i), recursiveTCMap, repeatedTCMap );
            out.write_short( typeCode.member_visibility(i) );
        }
    }

    public TCKind[] getSupportedTypeCodes()
    {
        return new TCKind[] {TCKind.tk_value};
    }
}
