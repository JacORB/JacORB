...
public class TCLTest extends TestCase {
	...
	TestUtils.runEvaluation(this, applicationContext_, event, expr);
    }
	...
	TCLNode fstNode = TCLParser.parse(fst);
	TCLNode sndNode = TCLParser.parse(snd);
	...
	TCLNode _root = 
	    (TCLNode)TCLParser.parse("$.first_name.value + 5");
	...
	for (int x=0; x<visitorTestExpressions_.length; ++x) {
	    TCLNode _root = TCLParser.parse(visitorTestExpressions_[x]);
	    _root.acceptPostOrder(new TCLCleanUp());
	    ComponentName _n = (ComponentName)_root.left();
	...
	TCLNode _root = TCLParser.parse(".1");
