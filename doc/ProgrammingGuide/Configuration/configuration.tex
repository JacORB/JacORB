...
JacORB does not attempt to configure the external logging system
(except for a small compatibility feature explained below).  This
means that it is left to the user to configure the chosen logging
system for their particular needs, for example, to choose log
verbosity or specify the names of log files.
...
There is also a special convenience feature for users upgrading from
previous JacORB versions: \emph{If} JDK logging is used, \emph{and}
one of the old properties {\tt jacorb.log.default.verbosity} or {\tt
  jacorb.logfile} is set, then JacORB configures the JDK logging
system accordingly at startup, resulting in a basic, usable logging
configuration.  This is only meant as a feature to ease upgrading.
For any advanced configuration, such as log file rotation, a separate
configuration file such as the provided sample file needs to be used.
...
following libraries need to be on the classpath: {\tt slf4j-api-1.5.6.jar},
{\tt slf4j-log4j-1.5.6.jar}, and {\tt log4j-1.2.15.jar}.

The JacORB distribution only ships with the generic SLF4J library and
the JDK adapter library.  Other libraries need to be downloaded from
the SLF4J site.
...
The JDK logging backend can be configured by several means, but the
most common way is to use a separate configuration file and specify
that file as a command line parameter to the JVM:

\begin{verbatim}
-Djava.util.logging.config.file=/path/to/config-file
\end{verbatim}

The configuration file itself contains definitions for log handlers
(targets), log file names, and the log verbosity (level) for the
individual loggers.  A sample file is provided in the JacORB
distribution, under {\tt
  JACORB\_HOME/etc/logging\_properties.template}.
