...
side effect, this duplication is a problem.


The solution is to use the following configuration property:
\begin{verbatim}
jacorb.use_reply_groups = on|off
\end{verbatim}
A reply group is a collection of reply receivers that are associated with a
specific profile. A single Delegate may have multiple active reply groups, a
Location Forward handled by a reply receiver will only impact the members of its
group. Use of the reply groups can have a slight performance hit as group
management is synchronized. This will be seen when a new group is created, which
will be the first time a Delegate is used, or in response to a Location Forward
message. Groups are removed from the collection when the last member is removed.

The default behavior is to not use profile-discriminated reply groups, rather to
use a single group for all reply receivers. This is the traditional behavior of
the Delegate.
...
\hline
\verb"jacorb.use_reply_group"
\verb"s" &
Use Profile Descriminated Reply Groups rather than a single group for all ReplyReceivers;
this is the default Delegate behaviour. & boolean & off\\
\hline
\end{longtable}
\end{small}
