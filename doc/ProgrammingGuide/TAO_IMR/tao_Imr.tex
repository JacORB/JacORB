...
ImR then responds with a LOCATION_FORWARD message which instructs 
...
COMM_FAILURE exception, a client may decide to retry by invoking 
...
In addition, the ImR Utility {\tt tao_imr} can be used to register the 
...
\begin{itemize}
   \item JACORB\_HOME and properties file
    ...
    \item LD\_LIBRARY_PATH=$ACE\_ROOT/lib
    \item DYLD\_LIBRARY\_PATH=$ACE_\ROOT/lib (needed for MACOS)
\end{itemize}

There are helper scripts in the {\tt $JACORB_HOME/test/orbreinvoke/tao_imr} 
...
\cmdline{\tt ./run\tao\_imr\_locator.sh}.
...
\cmdline{export ImplRepoServiceIOR=///tmp/tao_imr\_locator.ior}

\subsection{Initiate The TAO ImR Activator {\tt tao\_imr\_activator}
...
\cmdline{\tt ./run\tao\_imr\_activator.sh}.
...
\cmdline{./run_SimpleClient.sh EchoServer1 [ImR hostname]}
...
\cmdline {\tt corbaloc:protocol:host_or_ip:port/ServerName} 
...
The full path to the ImR is: {\tt $TAO_ROOT/orbsvcs/ImplRepo\_Service/tao\_imr\_locator}.
...
\hline
\verb"-d" & Specify IMR specific debugging level, \{\tt0-2}. Default is {\tt 1}.\\
\hline
...
The full path to the tao_\imr\_activator is \{tt T$AO_ROOT/orbsvcs/ImplRepo_Service/tao\_imr\_activator}. 
...
\hline
\verb“-c command” & {\tt install}, {\tt remove} or {\tt install_no_imr} the Windows service. \\
\hline
...
\cmdline{tao\_imr\_activator.exe –d 2 –o activator.ior –c install_no_imr}
