...
The standard mechanism for specifying a timing policy for an object
is quite verbose: We need to stuff the time value into a CORBA Any,
let the ORB create a Policy object from that, and supply that as an
argument to {\tt \_set\_policy\_override()}.  As an example, if we
want to set a \emph{ReplyEndTimePolicy} that lies one second in the
future, this looks as follows:
...
To make this a little less verbose, JacORB's implementations of
policy values have constructors that allow you to create them right
away, without calling the ORB, and without going via an Any.  Thus,
the above becomes:
