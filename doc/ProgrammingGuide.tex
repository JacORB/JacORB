...
Xtradyne Technologies, Berlin, Germany\\
\{brose,noffke\}@xtradyne.com\\
\\
$Revision$
}
...
examples) you need to have the XML--based make tool ``Ant'' installed on your
machine.  Ant can be downloaded from {\tt http://jakarta.apache.org/ant}. All
make files ({\tt build.xml}) are written for this tool. To rebuild JacORB
completely, add {\tt \JacORBDir/classes} to your CLASSPATH, and type {\tt ant}
in the installation directory.  Optionally, you might want to do a {\tt ant
  clean} first.

In order to be able to use SSL you need a third party implementation of the
SSL protocol. We currently support:
\begin{enumerate}
...
\smallskip
\begin{figure}[htb]
\centerline{\epsfig{file=dynany.eps, width=\textwidth}}
