...
Request timeouts control how long to wait until a request is considered to be
lost. When set via the property {\tt jacorb.client.pending\_reply\_timeout},
all requests of the so-configured ORB will time out after the given number of
microseconds. When such a timeout occurs, a {\tt org.omg.CORBA. IMP\_LIMIT}
...
bewteen client and server will use the same connection. This saves resources
...
statistical data about the {\tt Transport}, the interface {\tt
...
{
    public Transport selectForClose( java.util.List transports );
}
...
The interface {\tt SelectionStrategy} has only the single method of {\tt
  selectForClose()}. This is called by the class {\tt TransportManager} when a
{\tt Transport} needs to be closed. The argument is a {\tt List} containing
objects of type {\tt Transport}. The call itself is synchronized in the {\tt
  TransportManager}, so no additional synchronization has to be done by the
implementor of {\tt SelectionStrategy}. When examining the {\tt Transport}s,
the strategy can get hold of the {\tt StatisticsProvider} via the method {\tt
  getStatisticsProvider()} of the interface {\tt Transport}. The strategy
implementor should take care only to return idle connections. While the
connection state is checked anyway while closing (it may have changed in the
meantime), it seems to be more efficient to avoid cycling through the
connections. When no suitable {\tt Transport} is available, the strategy may
return {\tt null}. The {\tt TransportManager} will then wait for a
configurable time, and try again. This goes on until a {\tt Transport} can be
closed.
  
  The interface {\tt StatisticsProvider} is used to collect statistical data
  about a {\tt Transport} and provide it to the {\tt SelectionStrategy}.
  Because the nature of this data may vary, there is no standard access to the
  data via the interface. Therefore, {\tt StatisticsProvider} and {\tt
    SelectionStrategy} usually need to be implemented together. Whenever a new
  {\tt Transport} is created\footnote{Currently, connection management is only
    implemented for the server side. Therefore, only accepted {\tt Transport}s
    will get a {\tt StatisticsProvider}}, a new {\tt StatisticsProvider}
  object is instanciated and stored with the {\tt Transport}\footnote{This is
    actually only done when a {\tt StatisticsProvider} is configured}. The
  {\tt StatisticsProvider} interface is oriented along the mode of use of the
  {\tt Transport}. For efficiency reasons, messages are not sent as one big
  byte array. Instead, they are sent piecewise over the wire. When such a
  chunk is sent, the method {\tt messageChunkSent(int size)} will be
  called. After the message has been completely sent, method {\tt flush()} is
  called. This whole process is synchronized, so all consecutive {\tt
  messageChunkSent}s until a {\tt flush()} form a single message. Therefore,
  no synchronization on this level is necessary. However, access to gathered
  statistical date by the {\tt SelectionStrategy} is concurrent, so care has
  to be taken. Receiving messages is done only on the whole, so there exists
  only one method, {\tt messageReceived(int size)}, to notify the {\tt
  StatisticsProvider} of such an event.
...
\item {\tt jacorb.connection.comm\_failure\_retries} This property sets the
  number of retries that will be made, until a {\tt COMM\_FAILURE} exception
  is propagated back up to the application code. See also section
  \ref{connection_management_limitations}.
\end{description}
...
Additionally, it is checked that no thread is currently reading from or
writing to the socket. Since blocking IO is used, the reader thread blocks on
the socket input stream until it has read data. The problem here is that there
is no way to determine if data is available at the socket, but the thread just
hasn't yet read it (or, is actually reading, but hasn't yet returned from the
call). If a connection close happens at this time, the client will see that
the connection is closed, but it hasn't yet received a reply.  Therefore, it
will consider the message lost and usually throw an exception.

To compensate for that, JacORB allows to to retry sending the message if a
socket error occurs. This behaviour is controlled by the property {\tt
  jacorb.connection.comm\_failure\_retries}. If set, JacORB will try to resend
the message the configured number of times, and only then propagate the
exception back up if retries are exceeded. If not set, all errors from the
socket layer will get right back into the application code.
...
\hline
\verb"jacorb.connection.comm"
\verb"_failure_retries" & This property sets the
  number of retries that will be made, until a {\tt COMM\_FAILURE} exception
  is propagated back up to the application code. Default is off. & integer \\
\hline
